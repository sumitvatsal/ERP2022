    ...
    public class StudentApiController : ApiController
    {
        ...
        public SubjectTeacherAllocatesAppp getclassteacherallocated(SubjectTeacherAllocates1 VAL)
        {
            ...
            try
            {
                ...
                else if (VAL.SectionID.Equals(0) || VAL.SectionID.Equals(null) || "".Equals(VAL.SectionID))
                {
                    obj.status = false;
                ...
                {
                    int avi = 0;
                    int SchoolIDD = Convert.ToInt32(VAL.SchoolID);
                    //Accounts Department
                    ...
                    var CLASSTEACHER = db.tblClassTeacherAllocations.Where(t => t.ClassID == VAL.ClassID && t.SectionID == VAL.SectionID && t.SchoolID == SchoolIDD).FirstOrDefault();
                    ...
                    ct.SchoolID = VAL.SchoolID;
                    ct.ClassID = VAL.ClassID;
                    ct.SectionID = VAL.SectionID;
                    ct.SubTeacherDetails = new List<SubjectTeacherDetails>();
        ...
        [System.Web.Http.HttpPost]
        public tblFeeStructure[] GetStructure(List<string> val)
        {
            List<tblFeeStructure> list = new List<tblFeeStructure>();
            try
            {
                int i = int.Parse(val[0]);
                var result = db.tblFeeStructures.Where(a => a.SchoolID == i && a.IsDeleted == null).ToList();

                foreach (var a in result)
                {
                    tblFeeStructure items = new tblFeeStructure();
                    items.StructureName = a.StructureName;
                    items.ID = a.ID;
                    list.Add(items);
                }
            }
        ...
        [System.Web.Http.HttpPost]
        public studentdetails FindStudentByIDapp(Parameters param)
        {
        ...
        public Student[] getAllStudentsbyclassSection(List<string> aa)
        {
            ...
            int rno = 0;
            DataTable dt = obj.getDataTable(@"select st.ID,SUserID,RegNo,c.CourseName Class,scd.School,st.FirstName,st.MiddleName,st.LastName,PicPath,RollNo, s.SectionName, st.SMSmobileNo, ss.Status StatusName, st.Status from TBLStudent
...
where st.IsDeleted is null and st.SchoolID='" + aa[0] + "' and st.ClassID='" + aa[1] + "' and st.SectionID='" + aa[2] + "' and st.IsDeleted is null order by st.FirstName");
        ...
        [System.Web.Http.HttpPost]
        public StudentLeaveApplyAPP getStudentsLeaveDetbyIDAPP(Parameters param)
        {
            //sqlHelper obj = new sqlHelper();
